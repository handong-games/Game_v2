    ...
    public sealed class AdventureSceneLoader : IScenePreloader
    {
        ...
        public async Awaitable Preload()
        {
            ...
            try
            {
                ...
                AsyncOperationHandle<AdventureModel> adventureHandle =
                ...
                AdventureModel adventure = await WaitForAddressableLoad(adventureHandle);
                CharacterModel character = await WaitForAddressableLoad(characterHandle);

                AsyncOperationHandle<CardDeckModel> cardDeckHandle =
                    _dbManager.CardDeck.LoadAsync(adventure.CardDeckId);

                assetHandles.Add(cardDeckHandle);

                CardDeckModel cardDeck = await WaitForAddressableLoad(cardDeckHandle);
